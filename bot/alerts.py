...
from config import ALERT_CPU_TEMP, ALERT_GPU_TEMP, ALERT_RAM_PCT, ALERT_VRAM_PCT, TELEGRAM_CHAT_ID
...
async def check_and_alert(context: ContextTypes.DEFAULT_TYPE):
    ...
    for hostname in hostnames:
        ...
        if gpu_temp is not None and gpu_temp > ALERT_GPU_TEMP:
            triggered.append(f"GPU temp: {gpu_temp} C (limite {ALERT_GPU_TEMP:.0f} C)")

        vram_pct = entry.get("vram_percent")
        if vram_pct is not None and vram_pct > ALERT_VRAM_PCT:
            triggered.append(f"VRAM: {vram_pct:.1f}% (limite {ALERT_VRAM_PCT:.0f}%)")

        ram_pct = entry.get("ram_percent")
        if ram_pct is not None and ram_pct > ALERT_RAM_PCT:
            triggered.append(f"RAM: {ram_pct:.1f}% (limite {ALERT_RAM_PCT:.0f}%)")
